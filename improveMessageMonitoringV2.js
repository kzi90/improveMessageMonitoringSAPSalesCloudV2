...
const downloadAllErrorMessages = async () => {
    try {
                ...
                message.subMessages.map(async (subMessage) => {
                    ...
                    inboundErrorsOnly +=
                        (newSubMessage?.error?.details?.[0]?.details?.map((d) => d.message)?.join("\n") ||
                            ...
                            newSubMessage?.error?.message) + " | " + message.id + " | " + subMessage.messageHeader.id + "\n";
        ...
        blob = new Blob(["Inbound-Errors:\n" + inboundErrorsOnly.trim() + "\n\nOutbound-Errors:\n" + outboundErrorsOnly.trim()], { type: "text/plain" });
    ...
    if (table) {
        ...
        rows.forEach((row) => {
            ...
            if (["Fehler", "ERROR"].includes(statusElement?.textContent?.trim()) && messageId) {
                const fetchUrl = `https://${window.location.host}/sap/c4c/api/v1/inbound-data-connector-service/messages/${messageId}/requests`;
                fetch(fetchUrl)
                    .then((response) => {
                        if (!response.ok) {
                    ...
                    .then((subMessagesWithErrorsIds) => {
                        ...
                        const fetchUrl = `https://${window.location.host}/sap/c4c/api/v1/inbound-data-connector-service/messages/${messageId}/requests/${subMessageIdToFetch}`;
                        fetch(fetchUrl)
                            .then((response) => {
                                if (!response.ok) {
                                    throw new Error(`HTTP-Fehler: ${response.status}`);
        ...
        rows.forEach((row) => {
            ...
            if (["Fehler", "ERROR"].includes(statusElement?.textContent?.trim()) && messageId) {
                const fetchUrl = `https://${window.location.host}/sap/c4c/api/v1/outbound-data-connector-service/messages/${messageId}/requests`;
                fetch(fetchUrl)
                    .then((response) => {
                        if (!response.ok) {
                            throw new Error(`HTTP-Fehler: ${response.status}`);
                    ...
                    .then((subMessagesWithErrorsIds) => {
                        const subMessageIdToFetch = subMessagesWithErrorsIds[0]?.subMessageId;
                        const fetchUrl = `https://${window.location.host}/sap/c4c/api/v1/outbound-data-connector-service/messages/${messageId}/requests/${subMessageIdToFetch}`;
                        fetch(fetchUrl)
                            .then((response) => {
                                if (!response.ok) {
                                    throw new Error(`HTTP-Fehler: ${response.status}`);
...
const processSubMessageTable = () => {
    const table = document
        ...
        ?.shadowRoot?.querySelector("sap-crm-table");
